...
edna_diversity = as.data.frame(occupancy.edna[,!c(colnames(occupancy.edna) %in% c("sample_type","site"))] %>% group_by(time,sample_id) %>% summarise_all(funs(sum)))
...
sum.comm = as.data.frame(test.comm %>% transmute(GW1 = GW1+eGW1,
                        GW2 = GW2+eGW2,
                        GW3 = GW3+eGW3,
                        GW4 = GW4+eGW4,
                        GW5 = eGW5,
...
rankindex(scale(test), sum.comm, c("euc","jac","man","bray","kul"))
...
plot(ord, type="n")
points(ord, select=which(rownames(ord$points) == c("GW1","GW2","GW3","GW4","GW5","GW6")), col="blue", pch=22)
points(ord, select=which(rownames(ord$points) == c("WB1","WB2","WB3","WB4","WB5","WB6")), col="green", pch=1)
points(ord, select=which(rownames(ord$points) == c("RB1","RB2","RB3","RB4","RB5","RB6")), col="red", pch=24)
orditorp(ord, display="sites", cex=1)
orditorp(ord, display="species", type="n")
plot(fit.oct, col="red")
dev.off()

#col=c("blue","blue","blue","blue","blue","blue","green","green","green","green","green","green","orange","orange","orange","orange","orange","orange"))
#points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="Oct"),col="blue",pch=22)
```
...
ordiellipse(meta.full, group=fact_type.full[,1],kind="ehull",lty=c(1,4,3),label=F,lwd=1)
...
cols_barplot = c("red","darkorange", "cyan3", "gold4", "plum2", "gray25", "yellow", "sandybrown",  "springgreen","tomato","palevioletred","darkcyan","goldenrod","khaki4","darkorchid3","gainsboro","tan4", "brown","grey48","black","darkgreen","khaki3","blue")
...
ggplot(occup.melt, aes(sample_type, value)) +
  ...
  labs(x="", y="Community composition (%)", title = "Taxa composition in samples (individual taxa grouped by high rank taxonomy)") +
  theme(legend.position = "right")
dev.off()

col_crustacea = c("#00CC99","#006666","#3399CC","#0000FF","#9966FF","#9900FF","#660066","#CC00CC","#FF00FF","#FF66FF","#CCCCCC","#999999")

ggplot(occup.melt[occup.melt$order == "Amphipoda",], aes(sample_type, value))+
  geom_bar(stat="identity", position="stack", aes(fill=variable)) +
  scale_fill_manual(values = col_crustacea, name = "") +
  theme_bw() +
  facet_grid(cols=vars(site), rows=vars(time)) +
  labs(x="", y="Crustacea taxa occupancy (count)", title = "Crustacea composition in samples") +
  theme(legend.position = "right")
