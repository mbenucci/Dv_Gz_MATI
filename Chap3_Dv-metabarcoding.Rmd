...
clean.guts = clean.guts[,!c(colnames(clean.guts) %in% c("Isopoda","Diptera","Cyclopidae","Dikerogammarus_villosus"))]
...
plot(meta.dv.guts, type="n", main="D.villosus gut contents NMDS", xlim=c(-1,2), ylim=c(-1,1))
...
text(0.5,-1,label=paste("Stress:", meta.dv.guts$stress))
dev.off()
```
...
adonis(adonis.edna~fact_site.edna*fact_time.edna, permutations = 1000, method="bray")
```
...
#ig = grep("C.floridanus|C.pseudogracilis|C.multisetosum|G.tigrinus|G.zaddachi", rownames(food.web))
...
#plotweb(food.web[ig,grep("GW", colnames(food.web))], y.width.low=0.05, y.width.high=0.1, low.spacing=0.05, adj.low=c(0.5,1.1), arrow="down")
dev.off()
