...
pack.list = c("AED","aod","ape","bipartite","devtools","dplyr","EcoSimR","ggplot2","ggsignif","gplots","grid","gridExtra","iNEXT","lattice","lme4","lmtest","MASS","mgcv","msa","network","phyloseq","plyr","reshape2","spaa","stringi","tidyr","UpSetR","vegan","zoo")
...
Based on the levels of contamination in the Positive samples, the threshold levels were decided to be respectively 0.5% (May data set) and 0.5% (October data set). In May data set we still detect _D. villosus_ contamination.
...
# Removing DNA read belonging to _D. villosus_ due to wide spread contamination
filter.may$Dikerogammarus_villosus <- 0
clean.may$Dikerogammarus_villosus <- 0
...
# Removing DNA read belonging to _D. villosus_ due to wide spread contamination
filter.oct$Dikerogammarus_villosus <- 0
clean.oct$Dikerogammarus_villosus <- 0
...
inext.edna = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% "sample_id")] %>% group_by(site, time) %>% summarise_all(funs(sum)))
...
png("images/eDNA_species_rarefactions.png", width = 1000, height = 800, units = "px")
ggiNEXT(inext.edna) +
  labs(y = "", x="Number of samples") +
...
#adonis(adonis.guts~fact_site.guts*fact_time.guts, permutations = 1000, method="euclidean")
#adonis(adonis.guts~fact_predator.guts*fact_time.guts, permutations = 1000, method="euclidean")
adonis(adonis.guts~fact_predator.guts*fact_site.guts*fact_time.guts, permutations = 1000, method="euclidean")
...
colnames(adonis.dvil)
adonis.dvil = adonis.dvil[,!c(colnames(adonis.dvil) %in% c("time","site","sample_type","predator_id"))]

adonis(adonis.dvil~fact_site.dvil*fact_time.dvil, permutations = 1000, method = "euclidean")
```
...
png("images/guts_predator_NMDS_euclidean.png", width = 500, height = 500, units = "px")
plot(meta.guts, type="n", main="gut contents NMDS")#, xlim=c(-2,1.5), ylim=c(-1.2,1.2))
...
```{r plotting guts nmds}
# full symbols for May, hollow symbols for October
png("images/guts_Dvil_only_NMDS_euclidean.png", width = 500, height = 500, units = "px")
plot(meta.dv.guts, type="n", main="D.villosus gut contents NMDS", xlim=c(-1.8,1), ylim=c(-1,1))
...
ordispider(meta.dv.guts,group=fact_site.dvil[,1],col=c("blue","green"),label=T)
ordiellipse(meta.dv.guts,group=fact_site.dvil[,1],kind="ehull",lty=c(1,5),label=F,lwd=1)
dev.off()
```

```{r PERMANOVA eDNA}
# eDNA PERMANOVA
adonis.edna = abund.edna[,!c(colnames(abund.edna) %in% c("time","site","sample_id"))]

fact_site.edna = matrix(c(abund.edna$site), ncol=1)
fact_time.edna = matrix(c(abund.edna$time), ncol=1)
fact_id.edna = matrix(c(abund.edna$sample_id), ncol=1)
fact_all.edna = matrix(c(abund.edna$site, abund.edna$time, abund.edna$sample_id), ncol=3)

adonis(adonis.edna~fact_site.edna*fact_time.edna*fact_id.edna, permutations = 10000, method="bray")
```
...
# full symbols for May, hollow symbols for October
png("images/eDNA_NMDS_bray-curtis.png", width = 500, height = 500, units = "px")
...
```{r eDNA and guts comparison}
# Sorting out variables and joining the two frames
abund.edna$sample_type = "eDNA"
abund.guts$sample_type = "gut"

abund.full = full_join(abund.guts, abund.edna)
abund.full[is.na(abund.full)] <- 0
adonis.full = abund.full[,!c(colnames(abund.full) %in% c("time","site","sample_id","predator_id","sample_type"))]

fact_site.full = matrix(c(abund.full$site), ncol=1)
fact_time.full = matrix(c(abund.full$time), ncol=1)
fact_type.full = matrix(c(abund.full$sample_type), ncol=1)
fact_all.full = matrix(c(abund.full$site, abund.full$time, abund.full$sample_type), ncol=3)

adonis(adonis.full~fact_site.full*fact_time.full*fact_type.full, permutations = 1000, method="euclidean")
```

```{r nmds based on distances and on abundance data}
# NMDS based on the abundance data (will give the species scores as well)
meta.abund.guts.edna = metaMDS(adonis.full, trymax=200, distance="euclidean")
```

```{r plotting eDNA nmds}
nmds.guts.edna = as.data.frame(scores(meta.abund.guts.edna))
nmds.guts.edna$id = rownames(nmds.guts.edna)
nmds.guts.edna$site = abund.full[,"site"]
nmds.guts.edna$time = abund.full[,"time"]
nmds.guts.edna$sample_type = abund.full[,"sample_type"]

nmds.sp.guts.edna = as.data.frame(scores(meta.abund.guts.edna,"species"))
nmds.sp.guts.edna$species = rownames(nmds.sp.guts.edna)

# full symbols for May, hollow symbols for October
png("images/guts-vs-eDNA_NMDS_euclidean.png", width = 800, height = 800, units = "px")
plot(meta.abund.guts.edna, type="n", main="gut contents vs. eDNA NMDS")#, xlim=c(-2.5,5), ylim=c(-1,1))
points(meta.abund.guts.edna, select=which(fact_site.full[,1]=="WB" & fact_time.full[,1]=="May"),col="black",pch=20)
points(meta.abund.guts.edna, select=which(fact_site.full[,1]=="WB" & fact_time.full[,1]=="Oct"),col="black",pch=1)
points(meta.abund.guts.edna, select=which(fact_site.full[,1]=="GW" & fact_time.full[,1]=="May"),col="blue",pch=22)
points(meta.abund.guts.edna, select=which(fact_site.full[,1]=="GW" & fact_time.full[,1]=="Oct"),col="blue",pch=15)
points(meta.abund.guts.edna, select=which(fact_site.full[,1]=="RB" & fact_time.full[,1]=="May"),col="red",pch=17)
points(meta.abund.guts.edna, select=which(fact_site.full[,1]=="RB" & fact_time.full[,1]=="Oct"),col="red",pch=24)
ordispider(meta.abund.guts.edna, group=fact_type.full[,1],col=c("blue","orange"),label=T)
ordiellipse(meta.abund.guts.edna, group=fact_type.full[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
dev.off()
```
...
```{r checking and removing duplication in species}
gw.kick$Dikerogammarus_sp. = gw.kick$Dikerogammarus_sp. + gw.kick$Dikerogammarus_spp.
gw.kick = gw.kick[,!c(colnames(gw.kick) %in% "Dikerogammarus_spp.")]
```
...
colnames(raref.kick)
raref.kick = raref.kick[,!c(colnames(raref.kick) %in% c("Trichoptera","Diptera","Oligochaeta","Ostracoda","Araneae","Coleoptera","Mollusca","Zygopotera","Plecoptera","Nematoda"))]
...
strip_labels=c("0"="Species Richness","1"="Shannon Diversity")
png("images/kick-samples_rarefactions.png", width = 1000, height = 800, units = "px")
ggiNEXT(inext.kick) +
...
adonis(adonis.kick~fact_site.kick*fact_time.kick, permutations = 1000, method="bray")
```
...
# full symbols for May, hollow symbols for October
png("images/kick-samples_NMDS_bray-curtis.png", width = 500, height = 500, units = "px")
...
adonis(adonis.kick.edna~fact_site.edki*fact_time.edki*fact_type.edki, permutations = 1000, method = "bray")
```
...
# full symbols for May, hollow symbols for October
png("images/kick-eDNA_NMDS_bray-curtis.png", width = 500, height = 500, units = "px")
plot(meta.kick.edna, type="n", main="kick-samples NMDS", xlim=c(-1,1), ylim=c(-0.6,0.6))
...
png("images/Dvil_niche_breadth.png", width=500, height=500, units="px")
...
#est.edna = as.data.frame(est.edna[,!c(colnames(est.edna) %in% c("sample_type","sample_id"))] %>% group_by(site, time) %>% summarise_all(funs(sum)))
#est.kick = as.data.frame(est.kick[,!c(colnames(est.kick) %in% c("REPLICATE","sample_type","sample_id"))] %>% group_by(site, time) %>% summarise_all(funs(sum)))

#est.web = full_join(est.edna, est.kick)
#est.web[is.na(est.web)] <- 0

#est.kick$Radix_balthica
#est.edna$Radix_balthica
#est.web$Radix_balthica

#colnames(est.web)
#est.web = as.data.frame(est.web %>% group_by(site, time) %>% summarise_all(funs(sum)))

#rownames(est.web) = paste(est.web$site, est.web$time, sep="_")
#est.web = est.web[,!c(colnames(est.web) %in% c("site","time"))]

#est.web = as.data.frame(t(est.web))
#rownames(est.web) = c("D.vil_GW_May","D.vil_GW_Oct","G.zad_RB_May","G.zad_RB_Oct","D.vil_WB_May","D.vil_WB_Oct")
#str(est.web)
#str(food.web)
#rownames(est.web)
#rownames(food.web)

#est.web = est.web %>% summarise_all(funs(sum))
#colnames(est.web)

png("images/bipartite_network.png", width=800, height=500, units="px")
plotweb(food.web, y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), low.lablength=5, col.high="white", low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.06, text.rot=90, col.interaction=c("blue","blue","orange","orange","green","green"))
dev.off()
```
...
```{r}
for (p in pack.list[2:length(pack.list)]){
 print(citation(p))
}
```
