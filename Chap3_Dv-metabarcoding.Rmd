...
data_structuring <- function(list_df){
  for (df in list_df){
    df = as.data.frame(df)
...
kick_df = list(gw.may,gw.oct,wb.may,wb.oct,rb.may,rb.oct)
data_structuring(kick_df)

View(gw.may)

colnames(gw.may)
test = gw.may
test = test[,!c(colnames(test) %in% c("ORDER","FAMILY"))]
spread(test, TAXA, NUMBERS, fill=0)
