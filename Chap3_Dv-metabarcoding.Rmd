...
adonis(dist.guts~fact_site.guts*fact_time.guts, permutations = 10000, method="euclidean")
adonis(dist.guts~fact_predator.guts*fact_time.guts, permutations = 10000, method="euclidean")
adonis(dist.guts~fact_predator.guts*fact_time.guts*fact_site.guts, permutations = 10000, method="euclidean")
```
...
ordispider(meta.ad.guts,group=fact_site.guts[,1],col=c("blue","orange","green"),label=T)
ordiellipse(meta.ad.guts,group=fact_site.guts[,1],kind="ehull",lty=c(1,3,5),label=F,lwd=1)
dev.off()
```
...
dist.edna = vegdist(adonis.edna, method="euclidean")

adonis(dist.edna~fact_site.edna*fact_time.edna*fact_id.edna, permutations = 10000, method="euclidean")
```

```{r nmds based on distances and on abundance data}
# NMDS based on the distance matrix
meta.edna = metaMDS(dist.edna, trymax=200, distance="euclidean")

# NMDS based on the abundance data (will give the species scores as well)
meta.ad.edna = metaMDS(adonis.edna, trymax=200, distance="euclidean")
```
