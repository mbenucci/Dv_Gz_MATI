...
```{r}
#### DNA READS OF CRANGONYX SPECIES TO BE SUMMED!!!!
```
...
clean.guts = subset(clean.guts, rowSums(clean.guts) > 0)
full.guts = cbind(clean.guts, info.guts)

# removing `sample_type` and `sample_id` variables and transforming detections into abundances
full.guts = full.guts[,!c(colnames(full.guts) %in% c("sample_type","sample_id"))]
full.guts[,!c(colnames(full.guts) %in% c("time","site","predator_id"))][full.guts[,!c(colnames(full.guts) %in% c("time","site","predator_id"))] > 0] <- 1
