...
sum(clean.may["cont13",])
clean.may = clean.may[!(rownames(clean.may) %in% "cont13"),]
filter.may = filter.may[!(rownames(filter.may) %in% "cont13"),]

clean.may = subset(clean.may, select = colSums(clean.may) > 0)
filter.may = subset(filter.may, select = colSums(filter.may) > 0)

clean.may = subset(clean.may, subset = rowSums(clean.may) > 0)
filter.may = subset(filter.may, subset = rowSums(filter.may) > 0)
```
...
bloct = grep("BL", rownames(filter.oct))
clean.oct[grep("BL", rownames(filter.oct)),]
clean.oct[grep("RB", rownames(filter.oct)),"Dikerogammarus_villosus"]

clean.oct = clean.oct[-bloct,]
filter.oct = filter.oct[-bloct,]
