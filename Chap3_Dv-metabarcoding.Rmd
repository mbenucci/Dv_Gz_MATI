...
filter.may$predator_id[filter.may$sample_id %in% c(miss.may[!miss.may %in% c("WB.507","RB.304","RB.303")])] = "Gammarus_zaddachi"
...
filter.may$predator_id
filter.oct$predator_id

pred_ratio_remove <- function(vector_of_rownames, input_frame){
  for (i in vector_of_rownames){
  pred_id = input_frame[i,"predator_id"]
  input_frame[i,pred_id] <- 0
  }
  return(input_frame)
}

# May
trow = rownames(filter.may[filter.may$predator_id != "water", c("site","sample_id","time","sample_type","predator_id")])
tcol = filter.may[filter.may$predator_ID != "water", "predator_id"]

#filter.may[trow,unique(tcol)]
pred_ratio_remove(trow, filter.may)

filter.may[rownames(filter.may) %in% "RB.302","predator_id"]

for (i in trow){
  filter.may[i, "predator_id"]
...
# Oct
urow = rownames(filter.oct[filter.oct$predator_ID != "water", 1:74])
ucol = filter.oct[filter.oct$predator_ID != "water", "predator_id"]

filter.oct[urow,unique(ucol)]
