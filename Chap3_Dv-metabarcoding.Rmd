...
occup.gut.edna.kick = full_join(abund.guts.site, abund.kick.edna)
...
ordispider(meta.full, group=fact_type.full[,1],col=c("black","gray60"),label=T)
ordiellipse(meta.full, group=fact_type.full[,1],kind="ehull",lty=c(1,3,4),label=F,lwd=1)
...
# 7. Food web bipartite network

For the bipartite network we use the gut contents data frame with the detections of preys. We need to first summarise the data by site and season, then we can transpose the data frame to have the prey along the rows and the predators along the columns. We simplify the names of the preys from the extended binomial name simply to allow a better reading in the bipartite plot.
...
# 8. Generating data summary table
