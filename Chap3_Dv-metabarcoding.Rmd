...
adonis(adonis.guts~fact_predator.guts*fact_time.guts, permutations = 9999, method="euclidean")
```
...
```{r plotting d.villosus guts nmds}
# full symbols for May, hollow symbols for October
png("R_images/guts_Dvil_time_NMDS_euclidean.png", width = 800, height = 800, units = "px")
...
dev.off()
# full symbols for May, hollow symbols for October
png("R_images/guts_Dvil_only_NMDS_euclidean.png", width = 800, height = 800, units = "px")
...
```{r plotting guts nmds}
# full symbols for May, hollow symbols for October
png("R_images/guts_predator_NMDS_euclidean.png", width = 800, height = 800, units = "px")
...
# full symbols for May, hollow symbols for October
png("R_images/eDNA_NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
...
# full symbols for May, hollow symbols for October
png("R_images/kick-samples_NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
...
# full symbols for May, hollow symbols for October
png("R_images/kick-eDNA_NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
plot(meta.kick.edna, type="n", main="a) Community NMDS\neDNA and kick-samples by sample type", xlim=c(-1,1), ylim=c(-1,1))
...
png("R_images/kick-eDNA_site-NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
plot(meta.kick.edna, type="n", main="b) Community NMDS\neDNA and kick-samples by site", xlim=c(-1,1), ylim=c(-1.1,1))
points(meta.kick.edna, select=which(fact_site.edki[,1]=="WB" & fact_time.edki[,1]=="May"),col="black",pch=20)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="WB" & fact_time.edki[,1]=="Oct"),col="black",pch=1)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="May"),col="blue",pch=22)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="Oct"),col="blue",pch=15)
...
```{r niche breadth of D.villosus}
niche.dv = abund.guts[abund.guts$predator_id == "D.vil",]
colnames(niche.dv)

# summing prey DNA reads by predator ID, site and time.
niche.dv = as.data.frame(niche.dv[,!c(colnames(niche.dv) %in% c("sample_type"))] %>% group_by(predator_id, time, site) %>% summarise_all(funs(sum)))
rownames(niche.dv) = paste(niche.dv$predator_id, niche.dv$site, niche.dv$time, sep="_")

## Null model with ra2 algorithm since this is not a niche overlap analysis
niche.dv.model = niche_null_model(speciesData=niche.dv[,!c(colnames(niche.dv) %in% c("site","time","predator_id"))], algo="ra2", metric="pianka", suppressProg=T, nReps=5000, saveSeed=T)

summary(niche.dv.model)
png("R_images/Dvil_niche_breadth.png", width=800, height=800, units="px")
plot(niche.dv.model, type="hist", main="Niche breadth of D.villosus across invaded sites")
dev.off()
```

```{r niche breadth index of D.villosus}
breadth.dv = as.data.frame(niche.dv[,!c(colnames(niche.dv) %in% c("time"))] %>% group_by(site,predator_id) %>% summarise_all(funs(sum)))
...
ggplot(guts_boot, aes(type, V1)) +
  ...
  theme_bw() +
  labs(y="Standardised Levin's index (boot-strapped)")
```
