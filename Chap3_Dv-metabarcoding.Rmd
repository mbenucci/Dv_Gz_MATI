...
inext.edna= t(abund.edna[,!c(colnames(abund.edna) %in% c("time","site"))])
...
ggplot() +
  geom_polygon(data=nmds.guts, aes(x=NMDS1,y=NMDS2, fill=site, group=site), alpha=0.3) +
...
```{r PERMANOVA eDNA}
# eDNA PERMANOVA
adonis.edna = abund.edna[,!c(colnames(abund.edna) %in% c("time","site"))]

fact_site.edna = matrix(c(abund.edna$site), ncol=1)
fact_time.edna = matrix(c(abund.edna$time), ncol=1)
fact_all.nmds = matrix(c(abund.edna$site, abund.edna$time), ncol=3)

dist.edna = vegdist(adonis.edna, method="euclidean")

adonis(dist.edna~fact_site.edna*fact_time.edna, permutations = 10000, method="euclidean")
```
