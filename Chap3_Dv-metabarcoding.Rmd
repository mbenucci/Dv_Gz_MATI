...
We can calculate now the mean read depth (±se) first across all samples, and secondly split by sample types (eDNA against gut contents).
...
```{r mean read depth + se by sample type}
#eDNA first run
mean(unlist(may.read.depth[grep("e", may.read.depth$variable),"value"]))
sd(unlist(may.read.depth[grep("e", may.read.depth$variable),"value"]))/sqrt(length(unlist(may.read.depth[grep("e", may.read.depth$variable),"value"])))

#gut contents first run
mean(unlist(may.read.depth[grep("^[^e]{1}[GW.]|^[^e]{1}[WB.]|^[^e]{1}[RB.]", may.read.depth$variable),"value"]))
sd(unlist(may.read.depth[grep("^[^e]{1}[GW.]|^[^e]{1}[WB.]|^[^e]{1}[RB.]", may.read.depth$variable),"value"]))/sqrt(length(unlist(may.read.depth[grep("^[^e]{1}[GW.]|^[^e]{1}[WB.]|^[^e]{1}[RB.]", may.read.depth$variable),"value"])))

#eDNA second run
mean(unlist(oct.read.depth[grep("e", oct.read.depth$variable),"value"]))
sd(unlist(oct.read.depth[grep("e", oct.read.depth$variable),"value"]))/sqrt(length(unlist(oct.read.depth[grep("e", oct.read.depth$variable),"value"])))

#gut contents second run
mean(unlist(oct.read.depth[grep("^[^e]{1}[GW.]|^[^e]{1}[WB.]|^[^e]{1}[RB.]", oct.read.depth$variable),"value"]))
sd(unlist(oct.read.depth[grep("^[^e]{1}[GW.]|^[^e]{1}[WB.]|^[^e]{1}[RB.]", oct.read.depth$variable),"value"]))/sqrt(length(unlist(oct.read.depth[grep("^[^e]{1}[GW.]|^[^e]{1}[WB.]|^[^e]{1}[RB.]", oct.read.depth$variable),"value"])))
```


Calculating the mean-sd value to be used as depth threshold, however the mean-sd values result negative. After checking for normality the read depth threshold was decided, and all samples with less than 500 reads are removed.
...
The filter coverage for all samples that are equal and greater than 500 reads coverage per samples were retained. The survival of the N samples was quite high in both data sets, with respectivelty 192 (88.9%) and 190 (95.9%) in May and October data sets.
...
```{r merginand PERMANOVA of kick-samples and eDNA frames}
