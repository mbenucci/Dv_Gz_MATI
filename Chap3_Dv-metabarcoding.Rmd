...
meta.ad.guts = metaMDS(adonis.guts, distance="bray")
```


```{r plotting guts nmds}
nmds.guts = as.data.frame(scores(meta.ad.guts))
...
nmds.sp.guts = as.data.frame(scores(meta.ad.guts,"species"))
nmds.sp.guts$species = rownames(nmds.sp.guts)

# full symbols for May, hollow symbols for October 
pdf("images/guts_predator_NMDS.pdf")
plot(meta.ad.guts, type="n", main="gut contents NMDS")#, xlim=c(-2,1.5), ylim=c(-1.2,1.2))
points(meta.ad.guts, select=which(fact_site.guts[,1]=="WB" & fact_time.guts[,1]=="May"),col="black",pch=20)
points(meta.ad.guts, select=which(fact_site.guts[,1]=="WB" & fact_time.guts[,1]=="Oct"),col="black",pch=1)
points(meta.ad.guts, select=which(fact_site.guts[,1]=="GW" & fact_time.guts[,1]=="May"),col="blue",pch=22)
points(meta.ad.guts, select=which(fact_site.guts[,1]=="GW" & fact_time.guts[,1]=="Oct"),col="blue",pch=15)
points(meta.ad.guts, select=which(fact_site.guts[,1]=="RB" & fact_time.guts[,1]=="May"),col="red",pch=17)
points(meta.ad.guts, select=which(fact_site.guts[,1]=="RB" & fact_time.guts[,1]=="Oct"),col="red",pch=24)
ordispider(meta.ad.guts,group=fact_predator.guts[,1],col=c("blue","orange"),label=T)
ordiellipse(meta.ad.guts,group=fact_predator.guts[,1],kind="ehull",lty=c(1,5),label=F,lwd=1)
dev.off()
```
...
fact_all.nmds = matrix(c(abund.edna$site, abund.edna$time, abund.edna$sample_id), ncol=3)
...
meta.edna = metaMDS(dist.edna, trymax=200, distance="bray")

# NMDS based on the abundance data (will give the species scores as well)
meta.ad.edna = metaMDS(adonis.edna, trymax=200, distance="bray")
```


```{r plotting eDNA nmds}
nmds.edna = as.data.frame(scores(meta.ad.edna))
...
nmds.sp.edna = as.data.frame(scores(meta.ad.edna,"species"))
nmds.sp.edna$species = rownames(nmds.sp.edna)

pdf("images/eDNA_NMDS.pdf")
plot(meta.ad.edna, type="n", main="eDNA NMDS", xlim=c(-2.5,4), ylim=c(-1,1))
points(meta.ad.edna, select=which(fact_site.edna[,1]=="WB" & fact_time.edna[,1]=="May"),col="black",pch=20)
points(meta.ad.edna, select=which(fact_site.edna[,1]=="WB" & fact_time.edna[,1]=="Oct"),col="black",pch=1)
points(meta.ad.edna, select=which(fact_site.edna[,1]=="GW" & fact_time.edna[,1]=="May"),col="blue",pch=22)
points(meta.ad.edna, select=which(fact_site.edna[,1]=="GW" & fact_time.edna[,1]=="Oct"),col="blue",pch=15)
points(meta.ad.edna, select=which(fact_site.edna[,1]=="RB" & fact_time.edna[,1]=="May"),col="red",pch=17)
points(meta.ad.edna, select=which(fact_site.edna[,1]=="RB" & fact_time.edna[,1]=="Oct"),col="red",pch=24)
ordispider(meta.ad.edna, group=fact_site.edna[,1],col=c("blue","orange","green"),label=T)
ordiellipse(meta.ad.edna, group=fact_site.edna[,1],kind="ehull",lty=c(1,3,5),label=F,lwd=1)
dev.off()
```
