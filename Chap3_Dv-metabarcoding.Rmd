...
The analysis was done in R (R Core Team, 2018) using the following environment and packages as specified below and in `log_session_packages.txt`.
...
We remove the taxonomy column from each input files and save them into separate files.
...
Calculating sample read depth for both input files, in order to quality check the overall sequencing.

```{r initial read depth, include=TRUE}
may.read.depth = as.data.frame(dv.may17)
oct.read.depth = as.data.frame(dv.oct17)

may.read.depth$taxa = rownames(may.read.depth)
oct.read.depth$taxa = rownames(oct.read.depth)
may.read.depth$time = "May"
oct.read.depth$time = "Oct"

may.read.depth = melt(may.read.depth)

# Sum of DNA reads by samples to calculate the sample read depth

may.read.depth = may.read.depth[,!c(colnames(may.read.depth) %in% "taxa")] %>% group_by(time, variable) %>% summarise_all(funs(sum))

oct.read.depth = melt(oct.read.depth)
oct.read.depth = oct.read.depth[,!c(colnames(oct.read.depth) %in% "taxa")] %>% group_by(time, variable) %>% summarise_all(funs(sum))

# Joining both frames and plot
...
Calculating mean and standard deviation (sd) of the samples read depth in each frame.

```{r}
# Removing samples and species that are empty
#dv.may17 = subset(dv.may17, select = colSums(dv.may17) > 0)
#dv.may17 = subset(dv.may17, subset = rowSums(dv.may17) > 0)

#dv.oct17 = subset(dv.oct17, select = colSums(dv.oct17) > 0)
#dv.oct17 = subset(dv.oct17, subset = rowSums(dv.oct17) > 0)

mean(colSums(dv.may17))
mean(colSums(dv.oct17))

sd(log10(colSums(dv.may17)))
sd(log10(colSums(dv.oct17)))

mean(log10(colSums(dv.may17)))-sd(log10(colSums(dv.may17)))
mean(log10(colSums(dv.oct17)))-sd(log10(colSums(dv.oct17)))

hist(log10(colSums(dv.may17)))
hist(log10(colSums(dv.oct17)))
shapiro.test(log10(colSums(dv.may17)))
shapiro.test(log10(colSums(dv.oct17)))

boxplot(log10(colSums(dv.may17)), log10(colSums(dv.oct17)), names=c("May 2017","Oct 2017"), ylab="N reads (log10)")
```

We calculate the filter coverage for all samples that are equale and greater than mean-2sd of the log10-transformed coverage per samples. We obtain that in both May and Oct 93% of the samples will pass the initial coverage filter.

```{r test_coverage, include=TRUE, echo=FALSE, fig.cap="coverage"}
m2sd.may = mean(log10(colSums(dv.may17)))-2*sd(log10(colSums(dv.may17)))
m2sd.oct = mean(log10(colSums(dv.oct17)))-2*sd(log10(colSums(dv.oct17)))

m2sd.may
m2sd.oct  

count(log10(colSums(dv.may17)) <= m2sd.may)[,2]/(count(log10(colSums(dv.may17)) > 0)[,2])
count(log10(colSums(dv.oct17)) <= m2sd.oct)[,2]/(count(log10(colSums(dv.oct17)) > 0)[,2])
```

```{r cleaning_frames}
cov.may17 = subset(dv.may17, select = log10(colSums(dv.may17)) >= m2sd.may)
cov.oct17 = subset(dv.oct17, select = log10(colSums(dv.oct17)) >= m2sd.oct)

# Cleaning empty taxa
cov.may17 = subset(cov.may17, subset = rowSums(dv.may17) > 0)
cov.oct17 = subset(cov.oct17, subset = rowSums(dv.oct17) > 0)
...
dvmay.clean = cov.may17[!(rownames(cov.may17)%in% disc.may),]
...
dvoct.clean = cov.oct17[!(rownames(cov.oct17)%in% disc.oct),]
```

```{r}
posmay = grep("POS", colnames(dvmay.clean))
pos.may = dvmay.clean[,posmay]

posoct = grep("POS", colnames(dvoct.clean))
pos.oct = dvoct.clean[,posoct]
...
Two of the positive samples in the October data frame contained high amount of contamination of _Crangonyx floridanus_ (648 reads over a total of 1399) and _Dikerogammarus villosus_ (2106 and 2681 reads over a total of respectively 3082 and 3746). In both cases of these taxa the threshold would be too high to obtain any information regarding the prey items. We decide 0.03% and 0.6% filters respectively for May and October. We apply the threshold to both data frames, the one transformed with the ratios and the one with cleaned number of reads.

```{r}
dvmay.clean = as.data.frame(t(dvmay.clean))

dvmay.filter = as.data.frame(dvmay.clean)
dvmay.filter = dvmay.filter[,]/rowSums(dvmay.filter)
dvmay.filter[is.na(dvmay.filter)] <- 0

dvmay.filter[][dvmay.filter <= 0.0003] <- 0
dvmay.clean[][dvmay.clean[,]/rowSums(dvmay.clean) <= 0.0003] <- 0

dvoct.clean = as.data.frame(t(dvoct.clean))

dvoct.filter = as.data.frame(dvoct.clean)
dvoct.filter = dvoct.filter[,]/rowSums(dvoct.filter)
dvoct.filter[is.na(dvoct.filter)] <- 0

dvoct.filter[][dvoct.filter <= 0.006] <- 0
dvoct.clean[][dvoct.clean[,]/rowSums(dvoct.clean) <= 0.006] <- 0

# Removing empty taxa

dvmay.clean = subset(dvmay.clean, select = colSums(dvmay.clean) > 0)
dvmay.filter = subset(dvmay.filter, select = colSums(dvmay.filter) > 0)

dvoct.clean = subset(dvoct.clean, select = colSums(dvoct.clean) > 0)
dvoct.filter = subset(dvoct.filter, select = colSums(dvoct.filter) > 0)
```


```{r test}
pnmay = grep("POS|NEG", rownames(dvmay.filter), ignore.case=T)
dvmay.filter = dvmay.filter[-pnmay,]
dvmay.clean = dvmay.clean[-pnmay,]

pnoct = grep("POS|NEG", rownames(dvoct.filter), ignore.case=T)
dvoct.filter = dvoct.filter[-pnoct,]
dvoct.clean = dvoct.clean[-pnoct,]

rownames(dvmay.clean) = gsub("[.]{1}", "", rownames(dvmay.clean))
rownames(dvmay.filter) = gsub("[.]{1}", "", rownames(dvmay.filter))

rownames(dvoct.clean) = gsub(".pl[0-9]{1}.{4,5}.nc.blast$", "", rownames(dvoct.clean))
rownames(dvoct.filter) = gsub(".pl[0-9]{1}.{4,5}.nc.blast$", "", rownames(dvoct.filter))

#dvmay.clean = subset(dvmay.clean, subset = rowSums(dvmay.clean) > 0)
#dvmay.filter = subset(dvmay.filter, subset = rowSums(dvmay.filter) > 0)

#dvoct.clean = subset(dvoct.clean, subset = rowSums(dvoct.clean) > 0)
#dvoct.filter = subset(dvoct.filter, subset = rowSums(dvoct.filter) > 0)
```

# checking for remaining known contaminations

_Harmonia axyridis_ is still present in one samples only (RB501) at a ratio of 0.0392 (approx. 4%) corresponding to 7 reads. Similarly _Osmia bicornis_ Despite the contamination filter didn't manage to exclude these contaminations, we can feel confident in removing these taxa without major changes in the final output.
Lastly we checked and remove any contamination samples. We have only `cont13` (lab contamination) which contains only 6 redas of _Dikerogammarus villosus_.


```{r remaining contaminations May}
# Harmonia axyridis contamination

count(dvmay.filter$Harmonia_axyridis > 0)
rownames(dvmay.filter)[dvmay.filter$Harmonia_axyridis > 0]
dvmay.clean$Harmonia_axyridis[rownames(dvmay.clean) %in% rownames(dvmay.filter)[dvmay.filter$Harmonia_axyridis > 0]]

dvmay.clean = dvmay.clean[,!(colnames(dvmay.clean) %in% "Harmonia_axyridis")]
dvmay.filter = dvmay.filter[,!(colnames(dvmay.filter) %in% "Harmonia_axyridis")]

# Osmia bicornis contamination

count(dvmay.filter$Osmia_bicornis > 0)
rownames(dvmay.filter)[dvmay.filter$Osmia_bicornis > 0]
dvmay.clean$Osmia_bicornis[rownames(dvmay.clean) %in% rownames(dvmay.filter)[dvmay.filter$Osmia_bicornis > 0]]

dvmay.clean = dvmay.clean[,!(colnames(dvmay.clean) %in% "Osmia_bicornis")]
dvmay.filter = dvmay.filter[,!(colnames(dvmay.filter) %in% "Osmia_bicornis")]

# cleaning contamination samples

sum(dvmay.clean["cont13",])
dvmay.clean = dvmay.clean[!(rownames(dvmay.clean) %in% "cont13"),]
dvmay.filter = dvmay.filter[!(rownames(dvmay.filter) %in% "cont13"),]
...
In the samples from October, the contamination situation was less problematic. No _Harmonia axyridis_ remained after filter threshold. _Osmia bicornis_ instead remained in only one samples (eGW4) at a ratio of 0.0188 (approx. 2%) corresponding to 28 reads. The DNA extraction blanks (`GW.BL1 & 2`, `WB.BL1 & 2`, `RB.BL1 & 2`) resulted containing lot of DNA reads:

* `GW.BL1` = 3399, `GW.BL2` = 1782
* `WB.BL1` = 6338, `WB.BL2` = 858
* `RB.BL1` = 1808, `RB.BL2` = 1991

Exploring these samples show that the only species contained in these samples is _Dikerogammarus villosus_. Less than ideal considering the 2 BLANKS from Rockland Broad (RB). The rest of the RB samples present though very low _D. villosus_ contamination.

```{r remaining contaminations Oct}
# Osmia bicornis contamination

count(dvoct.filter$Osmia_bicornis > 0)
rownames(dvoct.filter)[dvoct.filter$Osmia_bicornis > 0]
dvoct.clean$Osmia_bicornis[rownames(dvoct.clean) %in% rownames(dvoct.filter)[dvoct.filter$Osmia_bicornis > 0]]

colnames(dvoct.clean)

dvoct.clean = dvoct.clean[,!(colnames(dvoct.clean) %in% c("Osmia_bicornis","Psocoptera_sp._BOLD:AAN8452","Porcellio_scaber"))]
dvoct.filter = dvoct.filter[,!(colnames(dvoct.filter) %in% c("Osmia_bicornis","Psocoptera_sp._BOLD:AAN8452","Porcellio_scaber"))]
...
bloct = grep("BL", rownames(dvoct.filter))
dvoct.clean[grep("BL", rownames(dvoct.filter)),]
dvoct.clean[grep("RB", rownames(dvoct.filter)),"Dikerogammarus_villosus"]

dvoct.clean = dvoct.clean[-bloct,]
dvoct.filter = dvoct.filter[-bloct,]
...
We need to add other variables to the data frames; specifically the sampling season, the samples ID, the lake ID, and the predator ID (eDNA samples were labeled as "water"). We then split the frames into gut contents and eDNA samples.
For the `predator_ID`, we used different ration to calculate the ID. May we could use the taxa with ratio greater or equal to 50% within each samples, for October instead we could use only ID with ratio greater or equal to 90% before getting miss-assignment. The samples where the chances of miss-assignment was too high were added manually.

```{r adding variables}
dvmay.filter$time = "May"
dvoct.filter$time = "Oct"

dvmay.filter$sample_id = rownames(dvmay.filter)
dvoct.filter$sample_id = rownames(dvoct.filter)

# Nested gsub to keep only the 2 letter from each sample ID referring to the lake ID.

dvmay.filter$site = gsub("BL{1}$","",gsub("R{1}$","",gsub("[0-9]{1,3}","", gsub("e?","", dvmay.filter$sample_id))))
dvoct.filter$site = gsub("[.]BL{1}$","",gsub("R{1}$","",gsub("[0-9]{1,3}","", gsub("e?","", dvoct.filter$sample_id))))

# Adding the sample type (eDNA or gut contents)

ednamay = grep("e", dvmay.filter$sample_id)
dvmay.filter$sample_type = "NA"
dvmay.filter[ednamay,"sample_type"] = "eDNA"
dvmay.filter[-ednamay,"sample_type"] = "gut"

ednaoct = grep("e", dvoct.filter$sample_id)
dvoct.filter$sample_type = "NA"
dvoct.filter[ednaoct, "sample_type"] = "eDNA"
dvoct.filter[-ednaoct, "sample_type"] = "gut"

# Adding predator ID base on DNA ratio greater than 0.5 and 0.9

ednamay = grep("e", rownames(dvmay.filter))
#ednamay
predmay = which(dvmay.filter[-ednamay,1:87] >= 0.5, arr.ind=T)
rownames(predmay)
colnames(dvmay.filter[rownames(predmay),as.vector(predmay[,2])])

dvmay.filter$predator_ID = "NA"
dvmay.filter[ednamay,"predator_ID"] = "water"
dvmay.filter[rownames(predmay),"predator_ID"] = gsub("[.]?[0-9]{1,3}$", "", colnames(dvmay.filter[rownames(predmay),as.vector(predmay[,2])]))

ednaoct = grep("e", rownames(dvoct.filter))
#ednaoct
predoct = which(dvoct.filter[-ednaoct,1:74] >= 0.8, arr.ind=T)
rownames(predoct)
colnames(dvoct.filter[rownames(predoct),as.vector(predoct[,2])])

dvoct.filter$predator_ID = "NA"
dvoct.filter[ednaoct,"predator_ID"] = "water"
dvoct.filter[rownames(predoct),"predator_ID"] = gsub("[.]?[0-9]{1,3}$", "", colnames(dvoct.filter[rownames(predoct),as.vector(predoct[,2])]))

# Adding missing predators

count(dvmay.filter$predator_ID == "NA")
miss.may = rownames(dvmay.filter[dvmay.filter$predator_ID == "NA",])
dvmay.filter[rownames(dvmay.filter) %in% miss.may,]

dvmay.filter$predator_ID[rownames(dvmay.filter) == "RB303"] = "Corophium_multisetosum"
dvmay.filter$predator_ID[rownames(dvmay.filter) == "RB603"] = "Gammarus_zaddachi"
dvmay.filter$predator_ID[rownames(dvmay.filter) == "RB608"] = "Gammarus_zaddachi"

count(dvoct.filter$predator_ID == "NA")
miss.oct = rownames(dvoct.filter[dvoct.filter$predator_ID == "NA",])
dvoct.filter[rownames(dvoct.filter) %in% miss.oct,]
apply(dvoct.filter[rownames(dvoct.filter) %in% miss.oct,1:74],1,max)

dvoct.filter$predator_ID[rownames(dvoct.filter) == "GW401"] = "Dikerogammarus_villosus"
dvoct.filter$predator_ID[rownames(dvoct.filter) %in% c("RB301","RB302","RB303","RB304")] = "Gammarus_zaddachi"
dvoct.filter$predator_ID[rownames(dvoct.filter) == "RB412"] = "Crangonyx_pseudogracilis"
dvoct.filter$predator_ID[rownames(dvoct.filter) %in% c("RB606","RB613","RB616")] = "Gammarus_zaddachi"
```

We now need to extract the information from the predator ID, so that we can set their ratio values equal to zero. We are basically obtaining a matrix with the only values that regards prey items or potential prey items. The information of the predator ID will still be stored into the column `predator_ID`.

```{r removing predator ratio}
# May
trow = rownames(dvmay.filter[dvmay.filter$predator_ID != "water", 1:87])
tcol = dvmay.filter[dvmay.filter$predator_ID != "water", "predator_ID"]

dvmay.filter[trow,unique(tcol)]

for (i in trow){
  pred_id = dvmay.filter[i,"predator_ID"]
  dvmay.filter[i,pred_id] <- 0
}
...
# Oct
urow = rownames(dvoct.filter[dvoct.filter$predator_ID != "water", 1:74])
ucol = dvoct.filter[dvoct.filter$predator_ID != "water", "predator_ID"]

dvoct.filter[urow,unique(ucol)]

for (i in urow){
  pred_id = dvoct.filter[i,"predator_ID"]
  dvoct.filter[i,pred_id] <- 0
}
...
```{r}
write.csv(dvmay.filter, "R/May_filtered.csv")
write.csv(dvoct.filter, "R/Oct_filtered.csv")
```


We can now start plotting and running models!
...
ggplot(edna.full, aes(sample_id, value)) +
  geom_jitter() +
