...
write.csv(clean.guts, "R/gut_contents_reads.csv")
write.csv(clean.edna, "R/eDNA_reads.csv")
...
Furthermore the taxa `Isopoda`, that should have been assigned to _Asellus aquaticus_, can't really be used for our purposes because of our detected contamination from _Porcellio scaber_. The 1217 read belonging to Isopoda will need to be removed from the gut contents dataset. _Dikerogammarus villosus_
...
clean.guts = clean.guts[,!c(colnames(clean.guts) %in% c("Isopoda","Dikerogammarus_villosus","Diptera","Cyclopidae"))]
...
colnames(abund.kick)
abund.kick = abund.kick[,!c(colnames(abund.kick) %in% c("Trichoptera","Diptera","Oligochaeta","Ostracoda","Araneae","Coleoptera","Mollusca","Zygopotera","Plecoptera","Nematoda"))]
...
kick.summary = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("sample_id"))] %>% group_by(site,time, sample_type) %>% summarise_all(funs(sum)))

test = data.frame(c(guts.summary[,c(colnames(abund.guts) %in% c("sample_type","predator_id","site","time"))]),c(guts.summary[,!c(colnames(abund.guts) %in% c("sample_type","predator_id","site","time"))]))
View(test)
```
