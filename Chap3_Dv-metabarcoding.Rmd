...
ratio.guts = ratio.guts[,!c(colnames(ratio.guts) %in% c("sample_type"))]
ratio.guts[,!c(colnames(ratio.guts) %in% c("time","site","predator_id","sample_id"))][ratio.guts[,!c(colnames(ratio.guts) %in% c("time","site","predator_id","sample_id"))] > 0] <- 1

# rarefaction curves
abund.guts = as.data.frame(ratio.guts)
abund.guts.site = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_id"))] %>% group_by(time, site, predator_id) %>% summarise_all(funs(sum)))
...
dev.off()
```
# 3. Diversity analysis of eDNA samples
