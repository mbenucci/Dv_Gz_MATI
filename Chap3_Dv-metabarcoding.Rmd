...
#network.prey = rownames(food.web)
#rownames(food.web) <- NULL
...
est.edna = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_type","sample_id"))] %>% group_by(site, time) %>% summarise_all(funs(sum)))
est.kick = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("REPLICATE","sample_type","sample_id"))] %>% group_by(site, time) %>% summarise_all(funs(sum)))
