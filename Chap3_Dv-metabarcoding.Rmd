...
```{r PERMANOVA eDNA vs gut contents}
head(abund.edna)
head(abund.guts)

abund.edna$type = "eDNA"
abund.guts$type = "guts"

nmds.full = full_join(abund.edna, abund.guts)
nmds.full[is.na(nmds.full)] <- 0
nmds.full = nmds.full[,!c(colnames(nmds.full) %in% "predator_id")]

matrix.nmds = as.matrix(nmds.full[,!c(colnames(nmds.full) %in% c("time","site","type"))])

fact_site.nmds = matrix(c(nmds.full$site), ncol=1) 
fact_time.nmds = matrix(c(nmds.full$time), ncol=1)
fact_type.nmds = matrix(c(nmds.full$type), ncol=1)
fact_all.nmds = matrix(c(nmds.full$site, nmds.full$time, nmds.full$type), ncol=3)

dist.nmds = vegdist(matrix.nmds, method="bray")

adonis(dist.nmds~fact_site.nmds*fact_type.nmds, permutations = 10000, method="bray")
adonis(dist.nmds~fact_time.nmds*fact_type.nmds, permutations = 10000, method="bray")
adonis(dist.nmds~fact_site.nmds*fact_time.nmds, permutations = 10000, method="bray")
```

```{r}
meta.nmds = metaMDS(dist.nmds, zerodist=fail, trymax=200, distance="bray")
```

```{r}
plot(meta.nmds,type="n",
main="gut contents PERMANOVA")#, ylim=c(-0.7,0.9), xlim=c(-0.5,1))
points(meta.nmds, select=which(fact_site.nmds[,1]=="WB"),col="black",pch=1)
points(meta.nmds, select=which(fact_site.nmds[,1]=="GW"),col="blue",pch=22)
points(meta.nmds, select=which(fact_site.nmds[,1]=="RB"),col="red",pch=25)
ordispider(meta.nmds,group=fact_site.nmds[,1], col=c("green","orange","green4"),label=T)
ordiellipse(meta.nmds,group=fact_type.nmds[,1],kind="ehull", lty=c(1,3),label=F,lwd=2)
