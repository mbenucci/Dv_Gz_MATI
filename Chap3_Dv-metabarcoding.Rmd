...
richness.kick = occupancy.kick
richness.kick[,!c(colnames(richness.kick) %in% c("time","sample_id","site","sample_type"))][richness.kick[,!c(colnames(richness.kick) %in% c("time","sample_id","site","sample_type"))] > 0] <- 1

richness.kick = rowSums(richness.kick[,!c(colnames(richness.kick) %in% c("sample_id","site","time","sample_type"))])
...
shannon.kick = diversity(occupancy.kick[,!c(colnames(occupancy.kick) %in% c("site","sample_id","sample_type","time"))], index="shannon")
shannon.kick = cbind(shannon.kick, occupancy.kick[,c(colnames(occupancy.kick) %in% c("site","sample_id","sample_type","time"))])
...
shannon.amphipod = occupancy.kick[,c(grep("Dikerogammarus|Gammarus|Crangonyx|Corophium|site|sample_type|sample_id|time", colnames(occupancy.kick)))]
shannon.amphipod[grep("GW|WB", rownames(shannon.amphipod)), grep("Dikerogammarus", colnames(shannon.amphipod))] <- 0
shannon.amphipod[grep("RB", rownames(shannon.amphipod)), grep("Gammarus_zaddachi|Gammarus_sp.", colnames(shannon.amphipod))] <- 0

test = diversity(shannon.amphipod[,!c(colnames(shannon.amphipod) %in% c("site","sample_id","sample_type","time"))], index="shannon")
test = cbind(test,shannon.amphipod[,c(colnames(shannon.amphipod) %in% c("site","sample_id","sample_type","time"))])
colnames(test)[colnames(test) == "test"] <- "diversity"

png("R_images/shannon_kick-amphipod_boxplot.png", width=3000, height=3000, units="px", res=300)
test %>% mutate(site = factor(site, levels=c("GW","WB","RB"))) %>% 
  ggplot(aes(site, diversity)) +
  geom_boxplot(aes(fill=time), outlier.shape = NA, alpha=0.9) +
  geom_point(position=position_jitterdodge(dodge.width = 0.85), aes(fill=time), color="black", pch=21, size=2, alpha=0.5) +
  facet_grid(.~sample_type, scales = "free_x") +
  theme_bw() +
  labs(y="Species richness", x="", title="Shannon diversity of Amphipods")
dev.off()
```
...
melt.kick[melt.kick$order == "Amphipoda",] %>% 
  mutate(variable = factor(variable, levels=c("Corophium_sp.", "Corophium_curvispinum", "Crangonyx_sp.", "Crangonyx_floridanus", "Crangonyx_pseudogracilis", "Gammarus_sp.", "Gammarus_tigrinus", "Gammarus_zaddachi", "Dikerogammarus_sp.", "Dikerogammarus_villosus"))) %>% 
  mutate(site = factor(site, levels=c("GW","WB","RB"))) %>% 
  ggplot(aes(sample_id, value, group=time)) +
  geom_bar(stat="identity", position=position_dodge(1), aes(fill=variable)) +
