...
```{r adding predator}
# Adding predator ID based on the DNA ratio, If greater than 0.9
ednamay = grep("e", rownames(filter.may))
predmay = which(filter.may[-ednamay,1:87] >= 0.9, arr.ind=T)
rownames(predmay)
...
miss.may = rownames(filter.may[filter.may$predator_id == "NA",])
filter.may[rownames(filter.may) %in% miss.may,]

filter.may$predator_ID[rownames(filter.may) == "RB303"] = "Corophium_multisetosum"
filter.may$predator_ID[rownames(filter.may) == "RB603"] = "Gammarus_zaddachi"
filter.may$predator_ID[rownames(filter.may) == "RB608"] = "Gammarus_zaddachi"

count(filter.oct$predator_id == "NA")
miss.oct = rownames(filter.oct[filter.oct$predator_id == "NA",])
filter.oct[rownames(filter.oct) %in% miss.oct,]
apply(filter.oct[rownames(filter.oct) %in% miss.oct,1:74], 1, max)

filter.oct$predator_ID[rownames(filter.oct) == "GW401"] = "Dikerogammarus_villosus"
filter.oct$predator_ID[rownames(filter.oct) %in% c("RB301","RB302","RB303","RB304")] = "Gammarus_zaddachi"
filter.oct$predator_ID[rownames(filter.oct) == "RB412"] = "Crangonyx_pseudogracilis"
filter.oct$predator_ID[rownames(filter.oct) %in% c("RB606","RB613","RB616")] = "Gammarus_zaddachi"
```
