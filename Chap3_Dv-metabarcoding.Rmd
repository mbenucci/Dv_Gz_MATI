...
```{r PERMANOVA eDNA and gut contents}
...
meta.guts = metaMDS(comm=dist.guts, trymax=200, distance="euclidean")

# NMDS based on the abundance data (will give the species scores as well)
meta.ad.guts = metaMDS(comm=adonis.guts, trymax=200, distance="euclidean")
```
...
ggplot() +
  geom_text(data=nmds.sp.guts, aes(x=NMDS1,y=NMDS2, label=species)) +
  geom_point(data=nmds.guts, aes(x=NMDS1,y=NMDS2), size=3) +
  geom_text(data=nmds.guts, aes(x=NMDS1,y=NMDS2, label=id), size=6, vjust=0) +
  coord_equal() +
  theme_bw()
...
points(meta.guts, select=which(fact_site.guts[,1]=="WB"),col="black",pch=1)
points(meta.guts, select=which(fact_site.guts[,1]=="GW"),col="blue",pch=22)
points(meta.guts, select=which(fact_site.guts[,1]=="RB"),col="red",pch=25)
ordispider(meta.guts,group=fact_site.guts[,1], col=c("green","orange","green4"),label=T)
ordiellipse(meta.guts,group=fact_predaotr.guts[,1],kind="ehull", lty=c(1,3),label=F,lwd=2)

# NOTE: to find the right way to plot this data! (barplot?) 
# Next steps: to sort nmds between eDNA and guts! (not sufficient data?? to explore) overlap between guts and eDNA. Next section kick-samples.
