...
```{r}
richness.kick = cbind(richness.kick, occupancy.kick[,c(colnames(occupancy.kick) %in% c("sample_id","site","time"))])
#richness.edna = cbind(richness.edna, occupancy.edna[,c(c)])
  
ggplot(richness.kick, aes(site, richness.kick)) +
  geom_boxplot(aes(fill=time), outlier.shape = NA, alpha=0.3) +
...
```{r}
boxplot(c(gwmay_kickrich, gwoct_kickrich), c(gwmay_rich, gwoct_rich), c(wbmay_kickrich, wboct_kickrich), c(wbmay_rich, wboct_rich), c(rbmay_kickrich, rboct_kickrich), c(rbmay_rich, rboct_rich), ylab="Species richness", names=c("GW-kick","GW-eDNA","WB-kick","WB-eDNA","RB-kick","RB-eDNA"))
```
