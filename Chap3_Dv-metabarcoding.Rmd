...
abund.edna = gen.sp.edna[,!c(colnames(gen.sp.edna) %in% c("sample_type","sample_id","predator_id"))]
abund.edna[,!c(colnames(abund.edna) %in% c("time","site"))][abund.edna[,!c(colnames(abund.edna) %in% c("time","site"))] > 0] <- 1

# adding genus information to help in plotting
test.edna = as.data.frame(t(abund.edna))
#abund.edna$genus = gsub("(([_]{1}[A-Za-z0-9-]{+}){1,+}(/{0,1}|[.]{0,1})){0,+}$","", rownames(test.edna))
#gsub("([_]{1}[A-Za-z0-9-]{+}){1,+}(/{0,1}|[.]{0,1})){0,+}$","", rownames(test.edna))

melt.abund.edna = melt(abund.edna)
head(melt.abund.edna)
