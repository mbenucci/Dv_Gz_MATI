...
ordispider(meta.ad.guts,group=fact_site.guts[,1],col=c("green","orange","blue"),label=T)
...
```{r}
#ggplot() +
##  geom_text(data=nmds.sp.guts, aes(x=NMDS1,y=NMDS2, label=species), size=2) +
#  geom_point(data=nmds.guts, aes(x=NMDS1,y=NMDS2, shape=time, colour=site), size=3) +
#  geom_text(data=nmds.guts, aes(x=NMDS1,y=NMDS2, label=site), size=3, vjust=1) +
#  geom_polygon(data=nmds.guts, aes(x=NMDS1,y=NMDS2, group=site, fill=site), alpha=0.3) +
...
plot(meta.edna, type="n", main="eDNA NMDS", xlim=c(-2.5, 5), ylim=c(-1.5,1.5))
points(meta.edna, select=which(fact_site.edna[,1]=="WB"),col="black",pch=1)
points(meta.edna, select=which(fact_site.edna[,1]=="GW"),col="blue",pch=22)
points(meta.edna, select=which(fact_site.edna[,1]=="RB"),col="red",pch=25)
ordispider(meta.edna,group=fact_site.edna[,1], col=c("green","orange","green4"),label=T)
ordiellipse(meta.edna,group=fact_site.edna[,1],kind="ehull", lty=c(1,3),label=F,lwd=2)
```
