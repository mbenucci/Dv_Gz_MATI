...
niche.dv.model = niche_null_model(speciesData=niche.model[,!c(colnames(niche.model) %in% c("site","time","predator_id"))], algo="ra2", metric="pianka", suppressProg=T, nReps=5000, saveSeed=T)
...
```{r niche overlap between D.villosus and G.zaddachi}
niche.dvgz = abund.guts

# summing prey DNA reads by predator ID, site and time.
niche.overlap = as.data.frame(niche.dvgz[,!c(colnames(niche.dvgz) %in% c("sample_type"))] %>% group_by(site, time, predator_id) %>% summarise_all(funs(sum)))
rownames(niche.overlap) = paste(niche.overlap$predator_id, niche.overlap$site, niche.overlap$time, sep="_")

## Null model with ra2 algorithm since this is not a niche overlap analysis
niche.overlap.model = niche_null_model(speciesData=niche.overlap[,!c(colnames(niche.overlap) %in% c("site","time","predator_id"))], algo="ra3", metric="pianka", suppressProg=T, nReps=5000, saveSeed=T)

summary(niche.overlap.model)
png("images/Dvil-Gzad_niche_overlap.png", width=500, height=500, units="px")
plot(niche.overlap.model, type="hist", main="Niche overlap of D.villosus and G.zaddachi")
dev.off()
```
