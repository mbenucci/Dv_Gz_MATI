...
plot(meta.ad.edna, type="n", main="eDNA NMDS", xlim=c(-2.5, 5), ylim=c(-1.5,1.5))
...
```{r}
#ggplot() +
##  geom_text(data=nmds.sp.edna, aes(x=NMDS1,y=NMDS2, label=species), size=2) +
#  geom_point(data=nmds.edna, aes(x=NMDS1,y=NMDS2, shape=time, colour=site), size=3) +
#  geom_text(data=nmds.edna, aes(x=NMDS1,y=NMDS2, label=site), size=3, vjust=1) +
#  geom_polygon(data=nmds.edna, aes(x=NMDS1,y=NMDS2, fill=site, group=site), alpha=0.3) +
#  scale_colour_manual(values=c("GW"="blue","RB"="orange","WB"="purple")) +
#  coord_equal() +
#  theme_bw()
```
...
fact_id.full = matrix(c(abund.full$sample_type), ncol=1)
...
adonis(dist.full~fact_site.full*fact_time.full*fact_id.full, permutations = 10000, method="euclidean")
