...
pack.list = c("aod","ape","bipartite","boot","devtools","dplyr","EcoSimR","ggplot2","ggsignif","gplots","grid","gridExtra","iNEXT","lattice","lme4","lmtest","MASS","mgcv","msa","network","phyloseq","plyr","reshape2","spaa","stringi","tidyr","UpSetR","vegan","zoo")
...
abund.guts.site = as.data.frame(abund.guts %>% group_by(time, site, predator_id) %>% summarise_all(funs(sum)))
...
abund.edna = gen.sp.edna[,!c(colnames(gen.sp.edna) %in% c("predator_id"))]
abund.edna[,!c(colnames(abund.edna) %in% c("time","sample_id","site","sample_type"))][abund.edna[,!c(colnames(abund.edna) %in% c("time","sample_id","site","sample_type"))] > 0] <- 1
colnames(abund.edna)
...
inext.edna = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_id","sample_type"))] %>% group_by(site, time) %>% summarise_all(funs(sum)))
...
head(abund.edna)
abund.edna$sample_id
edna_diversity = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_type","site"))] %>% group_by(time,sample_id) %>% summarise_all(funs(sum)))

rownames(edna_diversity) = paste(edna_diversity$sample_id, edna_diversity$time, sep="_")
sp_richness = rowSums(edna_diversity[,!c(colnames(edna_diversity) %in% c("sample_id","time"))])

# GW May
gwmay_rich = sp_richness[grep("GW._May", names(sp_richness))]
mean(gwmay_rich)
sd(gwmay_rich)/sqrt(length(gwmay_rich))

# GW Oct
gwoct_rich = sp_richness[grep("GW._Oct", names(sp_richness))]
mean(gwoct_rich)
sd(gwoct_rich)/sqrt(length(gwoct_rich))

# RB May
rbmay_rich = sp_richness[grep("RB._May", names(sp_richness))]
mean(rbmay_rich)
sd(rbmay_rich)/sqrt(length(rbmay_rich))

# RB Oct
rboct_rich = sp_richness[grep("RB._Oct", names(sp_richness))]
mean(rboct_rich)
sd(rboct_rich)/sqrt(length(rboct_rich))

# WB May
wbmay_rich = sp_richness[grep("WB._May", names(sp_richness))]
mean(wbmay_rich)
sd(wbmay_rich)/sqrt(length(wbmay_rich))

# WB Oct
wboct_rich = sp_richness[grep("WB._Oct", names(sp_richness))]
mean(wboct_rich)
...
adonis.dvil = adonis.dvil[,!c(colnames(adonis.dvil) %in% c("time","site","sample_type","predator_id"))]

adonis(adonis.dvil~fact_site.dvil*fact_time.dvil, permutations = 9999, method = "euclidean")

# comparing D.vil and G.zad
adonis.guts = abund.guts[,!c(colnames(abund.guts) %in% c("time","site","sample_type","predator_id"))]
...
points(meta.dv.guts, select=which(fact_site.dvil[,1]=="GW" & fact_time.dvil[,1]=="May"),col="blue",pch=22)
points(meta.dv.guts, select=which(fact_site.dvil[,1]=="GW" & fact_time.dvil[,1]=="Oct"),col="blue",pch=15)
...
points(meta.guts, select=which(fact_site.guts[,1]=="GW" & fact_time.guts[,1]=="May"),col="blue",pch=22)
points(meta.guts, select=which(fact_site.guts[,1]=="GW" & fact_time.guts[,1]=="Oct"),col="blue",pch=15)
...
```{r PERMANOVA eDNA}
# eDNA PERMANOVA
adonis.edna = abund.edna[,!c(colnames(abund.edna) %in% c("time","site","sample_id","sample_type"))]

fact_site.edna = matrix(c(abund.edna$site), ncol=1)
fact_time.edna = matrix(c(abund.edna$time), ncol=1)

adonis(adonis.edna~fact_site.edna*fact_time.edna, permutations = 9999, method="bray")
```

```{r nmds based on distances and on abundance data}
# NMDS based on the abundance data (will give the species scores as well)
meta.edna = metaMDS(adonis.edna, distance="bray")
```


```{r plotting eDNA nmds}
# full symbols for May, hollow symbols for October
png("R_images/eDNA_NMDS_bray-curtis.png", width = 3000, height = 3000, units = "px", res = 300)
...
points(meta.edna, select=which(fact_site.edna[,1]=="GW" & fact_time.edna[,1]=="May"),col="blue",pch=22)
points(meta.edna, select=which(fact_site.edna[,1]=="GW" & fact_time.edna[,1]=="Oct"),col="blue",pch=15)
...
# 3. Community composition of kick-samples and comparison with eDNA
...
kick_diversity = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("sample_type","site","REPLICATE"))] %>% group_by(time,sample_id) %>% summarise_all(funs(sum)))

rownames(kick_diversity) = paste(kick_diversity$sample_id, kick_diversity$time, sep="_")
kick_diversity[,!c(colnames(kick_diversity) %in% c("time","sample_id"))][kick_diversity[,!c(colnames(kick_diversity) %in% c("time","sample_id"))] > 0] <- 1

sp_rich.kick = rowSums(kick_diversity[,!c(colnames(kick_diversity) %in% c("sample_id","time"))])

# GW May
gwmay_kickrich = sp_rich.kick[grep("GW._May", names(sp_rich.kick))]
mean(gwmay_kickrich)
sd(gwmay_kickrich)/sqrt(length(gwmay_kickrich))

# GW Oct
gwoct_kickrich = sp_rich.kick[grep("GW._Oct", names(sp_rich.kick))]
mean(gwoct_kickrich)
sd(gwoct_kickrich)/sqrt(length(gwoct_kickrich))

# RB May
rbmay_kickrich = sp_rich.kick[grep("RB._May", names(sp_rich.kick))]
mean(rbmay_kickrich)
sd(rbmay_kickrich)/sqrt(length(rbmay_kickrich))

# RB Oct
rboct_kickrich = sp_rich.kick[grep("RB._Oct", names(sp_rich.kick))]
mean(rboct_kickrich)
sd(rboct_kickrich)/sqrt(length(rboct_kickrich))

# WB May
wbmay_kickrich = sp_rich.kick[grep("WB._May", names(sp_rich.kick))]
mean(wbmay_kickrich)
sd(wbmay_kickrich)/sqrt(length(wbmay_kickrich))

# WB Oct
wboct_kickrich = sp_rich.kick[grep("WB._Oct", names(sp_rich.kick))]
mean(wboct_kickrich)
...
# Summarising kick-samples by sites
abund.kick = as.data.frame(abund.kick %>% group_by(site, REPLICATE, time, sample_type, sample_id) %>% summarise_all(funs(sum)))

# Sorting out variables and joining the two frames
abund.kick[is.na(abund.kick)] <- 0
adonis.kick = abund.kick[,!c(colnames(abund.kick) %in% c("time","sample_id","sample_type","site","REPLICATE"))]

fact_site.kick = matrix(c(abund.kick$site), ncol=1)
fact_time.kick = matrix(c(abund.kick$time), ncol=1)

adonis(adonis.kick~fact_site.kick*fact_time.kick, permutations = 9999, method="bray")
```

```{r}
meta.ab.kick = metaMDS(adonis.kick, trymax=200, distance="bray")
```

```{r NMDS plot of kick-samples}
# full symbols for May, hollow symbols for October
png("R_images/kick-samples_NMDS_bray-curtis.png", width = 3000, height = 3000, units = "px", res = 300)
plot(meta.ab.kick, type="n", main="kick-samples NMDS")#, xlim=c(-2.5,5), ylim=c(-1,1))
points(meta.ab.kick, select=which(fact_site.kick[,1]=="WB" & fact_time.kick[,1]=="May"),col="black",pch=20)
points(meta.ab.kick, select=which(fact_site.kick[,1]=="WB" & fact_time.kick[,1]=="Oct"),col="black",pch=1)
points(meta.ab.kick, select=which(fact_site.kick[,1]=="GW" & fact_time.kick[,1]=="May"),col="blue",pch=22)
points(meta.ab.kick, select=which(fact_site.kick[,1]=="GW" & fact_time.kick[,1]=="Oct"),col="blue",pch=15)
points(meta.ab.kick, select=which(fact_site.kick[,1]=="RB" & fact_time.kick[,1]=="May"),col="red",pch=17)
points(meta.ab.kick, select=which(fact_site.kick[,1]=="RB" & fact_time.kick[,1]=="Oct"),col="red",pch=24)
ordispider(meta.ab.kick, group=fact_site.kick[,1],col=c("blue","orange","green"),label=T)
ordiellipse(meta.ab.kick, group=fact_site.kick[,1],kind="ehull",lty=c(1,3,5),label=F,lwd=1)
text(-0.5,-0.4, label=paste0("Stress: ", round(meta.ab.kick$stress,6)))
dev.off()
```

```{r merging kick-samples and eDNA frames}
abund.kick.edna = full_join(abund.edna, abund.kick)
abund.kick.edna[is.na(abund.kick.edna)] <- 0

adonis.kick.edna = abund.kick.edna[,!c(colnames(abund.kick.edna) %in% c("site","sample_id","sample_type","time","REPLICATE"))]

fact_site.edki = matrix(c(abund.kick.edna$site), ncol=1)
fact_time.edki = matrix(c(abund.kick.edna$time), ncol=1)
fact_type.edki = matrix(c(abund.kick.edna$sample_type), ncol=1)

adonis(adonis.kick.edna~fact_site.edki*fact_time.edki*fact_type.edki, permutations = 9999, method = "bray")
```

```{r}
meta.kick.edna = metaMDS(adonis.kick.edna, trymax = 200, methods = "bray")
```


```{r NMDS plot of kick-samples and eDNA}
# full symbols for May, hollow symbols for October
png("R_images/kick-eDNA_NMDS_bray-curtis.png", width = 3000, height = 3000, units = "px", res = 300)
plot(meta.kick.edna, type="n", main="Community NMDS\neDNA and kick-samples by sample type", xlim=c(-1,1), ylim=c(-1,1))
...
points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="May"),col="blue",pch=22)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="Oct"),col="blue",pch=15)
...
text(0.5,-0.8, label=paste0("Stress: ",round(meta.kick.edna$stress, 6)))
dev.off()
```

```{r NMDS plot by site of kick-samples and eDNA}
png("R_images/kick-eDNA_site-NMDS_bray-curtis.png", width = 3000, height = 3000, units = "px", res = 300)
plot(meta.kick.edna, type="n", main="Community NMDS\neDNA and kick-samples by site", xlim=c(-1,1), ylim=c(-1.1,1))
points(meta.kick.edna, select=which(fact_site.edki[,1]=="WB" & fact_time.edki[,1]=="May"),col="black",pch=20)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="WB" & fact_time.edki[,1]=="Oct"),col="black",pch=1)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="May"),col="blue",pch=22)
points(meta.kick.edna, select=which(fact_site.edki[,1]=="GW" & fact_time.edki[,1]=="Oct"),col="blue",pch=15)
...
orditorp(meta.kick.edna, display="species")
ordispider(meta.kick.edna, group=fact_site.edki[,1],col=c("blue","orange","green"),label=T)
ordiellipse(meta.kick.edna, group=fact_site.edki[,1],kind="ehull",lty=c(1,3),label=F,lwd=1)
text(0.5,-0.8, label=paste0("Stress: ",round(meta.kick.edna$stress, 6)))
dev.off()
```
...
```{r setting up niche data frame of D.villosus}
niche.dv = abund.guts[abund.guts$predator_id == "D.vil",]
colnames(niche.dv)

# summing prey DNA reads by predator ID and site.
breadth.dv = as.data.frame(niche.dv[,!c(colnames(niche.dv) %in% c("time","sample_type"))] %>% group_by(site,predator_id) %>% summarise_all(funs(sum)))

rownames(breadth.dv) = paste(breadth.dv$predator_id, breadth.dv$site, sep="_")
```

```{r niche breadth index of D.villosus}
breadth.dv = t(breadth.dv[,!c(colnames(breadth.dv) %in% c("site","predator_id"))])
...
```{r bootstrap on niche breadth of d.villosus}
standardised_levins = function(data,i){
  pi <- table(data[i])/length(data[i])
  B = 1/sum(pi^2)
  (B-1)/(35-1)
}

shannon = function(data,i){
  pi <- table(data[i])/length(data[i])
  -sum(log(pi)*pi)
}

dvil_gw1 = breadth.dv[,1]
dvil_wb2 = breadth.dv[,2]

c_gw1 = rep(1:length(dvil_gw1),dvil_gw1)
c_wb2 = rep(1:length(dvil_wb2),dvil_wb2)

b_gw1 = boot(c_gw1, standardised_levins, R=1000)
b_gw1

b_wb2 = boot(c_wb2, standardised_levins, R=1000)
b_wb2

shan_gw1 = boot(c_gw1, shannon, R=1000)
shan_gw1

shan_wb2 = boot(c_wb2, shannon, R=1000)
shan_wb2
```

```{r}
t.test(b_gw1$t, b_wb2$t)
```


```{r plot of levins bootstrap}
boot_bgw1 = as.data.frame(b_gw1$t)
boot_bwb2 = as.data.frame(b_wb2$t)

boot_bgw1$type = "D.vil_GW"
boot_bwb2$type = "D.vil_WB"

guts_boot = full_join(boot_bgw1, boot_bwb2)

ggplot(guts_boot, aes(type, V1)) +
  geom_boxplot() +
  geom_jitter(alpha=0.3) +
  theme_bw() +
  labs(y="Standaridsed Levin's index", x="Predator", title="Bootstrapped Standardised Levin's index for D. villosus")
```


```{r niche breadth of D.villosus vs G.zaddachi}
breadth.dvgz = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("time"))] %>% group_by(predator_id, site) %>% summarise_all(funs(sum)))
...
```{r bootstrap of D.vil niche vs G.zad niche}
dvil1 = breadth.dvgz[,1]
dvil2 = breadth.dvgz[,2]
gzad = breadth.dvgz[,3]

c_dvil1 = rep(1:length(dvil1),dvil1)
c_dvil2 = rep(1:length(dvil2),dvil2)
c_gzad = rep(1:length(gzad),gzad)

b_dvil1 = boot(c_dvil1, standardised_levins, R=1000)
b_dvil1

b_dvil2 = boot(c_dvil2, standardised_levins, R=1000)
b_dvil2

b_gzad = boot(c_gzad, standardised_levins, R=1000)
b_gzad

shan_dvil1 = boot(c_dvil1, shannon, R=1000)
shan_dvil1

shan_dvil2 = boot(c_dvil2, shannon, R=1000)
shan_dvil2

shan_gzad = boot(c_gzad, shannon, R=1000)
shan_gzad
```


```{r plot of levins bootstrap for dvil and gzad}
boot_bdvil1 = as.data.frame(b_dvil1$t)
boot_bdvil2 = as.data.frame(b_dvil2$t)
boot_bgzad = as.data.frame(b_gzad$t)

boot_bdvil1$type = "D.villosus_GW"
boot_bdvil2$type = "D.villosus_WB"
boot_bgzad$type = "G.zaddachi"

dv.gz_boot = full_join(boot_bdvil1, boot_bdvil2)
dv.gz_boot = full_join(dv.gz_boot, boot_bgzad)

ggplot(dv.gz_boot, aes(type, V1)) +
  geom_boxplot() +
  geom_jitter(alpha=0.3) +
  theme_bw() +
  labs(y="Standaridsed Levin's index", x="Predator", title="Bootstrapped Standardised Levin's index for D. villosus against G. zaddachi")
```
...
food.web = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("sample_type"))] %>% group_by(site, time, predator_id) %>% summarise_all(funs(sum)))
rownames(food.web) = paste(food.web$predator_id, food.web$site, food.web$time, sep="_")

food.web = food.web[,!c(colnames(food.web) %in% c("site","time","predator_id"))]
food.web = as.data.frame(t(food.web))

rownames(food.web) = gsub("[a-z]{4,12}_",".", rownames(food.web))

# Grafham Water
png("R_images/bipartite_networks_Grafham.png", width=800, height=500, units="px")
plotweb(food.web[,grep("GW", colnames(food.web))], y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lablength=5, low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.07, text.rot=90, col.high="white", col.interaction=c("blue"))
dev.off()

# Wroxham Broad
png("R_images/bipartite_networks_Wroxham.png", width=800, height=500, units="px")
plotweb(food.web[,grep("WB", colnames(food.web))], y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lablength=5, low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.07, text.rot=90, col.high="white", col.interaction=c("green"))
dev.off()

# Rockland Broad
png("R_images/bipartite_networks_Rockland.png", width=800, height=500, units="px")
plotweb(food.web[,grep("RB", colnames(food.web))], method="cca", y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lablength=5, low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.07, text.rot=90, col.high="white", col.interaction=c("orange"))
dev.off()

# Producing the bipartite network
png("R_images/bipartite_network.png", width=800, height=500, units="px")
plotweb(food.web, y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lablength=5, low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.07, text.rot=90, col.high="white", col.interaction=c("blue","blue","orange","orange","green","green"))
dev.off()
```

```{r bipartite network with estimates of preys}
# Setting up the estimate vectors as taxa distribution
est.edna = abund.edna
est.kick = abund.kick

est.edna[,!c(colnames(est.edna) %in% c("site","time","sample_type","sample_id"))][est.edna[,!c(colnames(est.edna) %in% c("site","time","sample_type","sample_id"))] > 0] <- 1
est.kick[,!c(colnames(est.kick) %in% c("site","time","sample_type","sample_id","REPLICATE"))][est.kick[,!c(colnames(est.kick) %in% c("site","time","sample_type","sample_id","REPLICATE"))] > 0] <- 1

est.edna = as.data.frame(est.edna[,!c(colnames(est.edna) %in% c("sample_type","sample_id","site","time"))] %>% summarise_all(funs(sum)))
est.kick = as.data.frame(est.kick[,!c(colnames(est.kick) %in% c("REPLICATE","sample_type","sample_id","site","time"))] %>% summarise_all(funs(sum)))

# Joining eDNA and kick-samples estimates
est.web = full_join(est.edna, est.kick)
est.web[is.na(est.web)] <- 0

est.web = as.matrix(est.web)
names(est.web) = colnames(est.web)

# Producing the bipartite network with prey abundances
png("R_images/bipartite_network_estimated.png", width=800, height=500, units="px")
plotweb(food.web, y.width.low=0.1, y.width.high=0.08,	y.lim=c(0,3), labsize=2, low.lablength=5,  low.lab.dis=0.1, high.lab.dis=0.1, low.spacing=0.012, high.spacing = 0.12, text.rot=90, low.abun=est.web, low.abun.col="grey", col.high="white", col.interaction=c("blue","blue","orange","orange","green","green"))
dev.off()
```

```{r generating data summary table}
guts.summary = as.data.frame(abund.guts %>% group_by(site,time,predator_id) %>% summarise_all(funs(sum)))
guts.summary$sample_type = "gut contents"
edna.summary = as.data.frame(abund.edna[,!c(colnames(abund.edna) %in% c("sample_id"))] %>% group_by(site,time, sample_type) %>% summarise_all(funs(sum)))
