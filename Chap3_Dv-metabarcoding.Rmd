...
dist.full = vegdist(adonis.full, method="bray")

adonis(dist.full~fact_site.full*fact_time.full*fact_type.full, permutations = 1000, method="euclidean")
```
...
gw.kick$site = paste0(gw.kick$LOCATION, gw.kick$REPLICATE)
wb.kick$site = paste0(wb.kick$LOCATION, wb.kick$REPLICATE)
rb.kick$site = paste0(rb.kick$LOCATION, rb.kick$REPLICATE)
```
...
adonis.kick = abund.kick[,!c(colnames(abund.kick) %in% c("time","site","sample_type","LOCATION","REPLICATE"))]

fact_site.kick = matrix(c(abund.kick$LOCATION), ncol=1)
