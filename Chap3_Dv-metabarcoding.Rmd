...
occupancy.kick = as.data.frame(abund.kick[,!c(colnames(abund.kick) %in% c("sample_type","REPLICATE"))] %>% group_by(time,site,sample_id) %>% summarise_all(funs(sum)))

rownames(occupancy.kick) = paste(occupancy.kick$sample_id, occupancy.kick$time, sep="_")
occupancy.kick[,!c(colnames(occupancy.kick) %in% c("time","sample_id","site"))][occupancy.kick[,!c(colnames(occupancy.kick) %in% c("time","sample_id","site"))] > 0] <- 1
occupancy.kick[is.na(occupancy.kick)] <- 0

richness.kick = rowSums(occupancy.kick[,!c(colnames(occupancy.kick) %in% c("sample_id","site","time"))])
...
adonis.kick = occupancy.kick[,!c(colnames(occupancy.kick) %in% c("time","sample_id","site"))]
...
abund.kick.edna = full_join(abund.edna, occupancy.kick)#abund.kick)
...
#adonis.kick.edna = vegdist(as.matrix(adonis.kick.edna))
...
# full symbols for May, hollow symbols for October
test = rownames(meta.kick.edna$species)

for (s in rownames(meta.kick.edna$species)){
  if (grepl("sp.", s) == F){
    rownames(meta.kick.edna$species)[grep(s, rownames(meta.kick.edna$species))] <- gsub("^[a-z]{3,13}_",".", s)
  } else {
    rownames(meta.kick.edna$species)[grep(s, rownames(meta.kick.edna$species))] <- s
  }
}

#rownames(meta.kick.edna$species)[grep("Placobdella_costata", rownames(meta.kick.edna$species))]
View(test)
View(rownames(meta.kick.edna$species))
