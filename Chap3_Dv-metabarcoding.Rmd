...
# eDNA PERMANOVA
# abund.edna$sample_id
rownames(abund.edna)=paste(abund.edna$sample_id, abund.edna$time, sep="_")
...
# 3. Community composition from kick-samples.
...
occupancy.kick[is.na(occupancy.kick)] <- 0

richness.kick = rowSums(occupancy.kick[,!c(colnames(occupancy.kick) %in% c("sample_id","site","time","sample_type"))])

# GW May
gwmay_kickrich = richness.kick[grep("GW._May", names(richness.kick))]
mean(gwmay_kickrich)
sd(gwmay_kickrich)/sqrt(length(gwmay_kickrich))

# GW Oct
gwoct_kickrich = richness.kick[grep("GW._Oct", names(richness.kick))]
mean(gwoct_kickrich)
sd(gwoct_kickrich)/sqrt(length(gwoct_kickrich))

# RB May
rbmay_kickrich = richness.kick[grep("RB._May", names(richness.kick))]
mean(rbmay_kickrich)
sd(rbmay_kickrich)/sqrt(length(rbmay_kickrich))

# RB Oct
rboct_kickrich = richness.kick[grep("RB._Oct", names(richness.kick))]
mean(rboct_kickrich)
sd(rboct_kickrich)/sqrt(length(rboct_kickrich))

# WB May
wbmay_kickrich = richness.kick[grep("WB._May", names(richness.kick))]
mean(wbmay_kickrich)
sd(wbmay_kickrich)/sqrt(length(wbmay_kickrich))

# WB Oct
wboct_kickrich = richness.kick[grep("WB._Oct", names(richness.kick))]
mean(wboct_kickrich)
...
points(meta.kick, select=which(fact_site.kick[,1]=="GW" & fact_time.kick[,1]=="May"),col="blue",pch=22)
points(meta.kick, select=which(fact_site.kick[,1]=="GW" & fact_time.kick[,1]=="Oct"),col="blue",pch=15)
...
#4. Community composition of eDNA and kick-samples

```{r merging kick-samples and eDNA frames}
abund.kick.edna = full_join(abund.edna, occupancy.kick)
...
# 5. Comparison of gut contents detection against community composition
...
abund.guts$sample_type = "guts"
abund.kick.edna$sample_type

occup.gut.edna.kick = full_join(abund.guts, abund.kick.edna)
occup.gut.edna.kick[is.na(occup.gut.edna.kick)] <- 0

occup.gut.edna.kick = as.data.frame(occup.gut.edna.kick[,!c(colnames(occup.gut.edna.kick) %in% c("predator_id","sample_id"))] %>% group_by(sample_type,site,time) %>% summarise_all(funs(sum)))
...
adonis(adonis.guts.edna.kick~fact_type.full, permutations = 9999, method = "jaccard")
```

```{r}
meta.full = metaMDS(adonis.guts.edna.kick, trymax = 200, methods = "Euclidean")
```
...
#png("R_images/kick-eDNA_NMDS_jaccard.png", width = 3000, height = 3000, units = "px", res = 300)
plot(meta.full, type="n", main="Community NMDS\neDNA and kick-samples by sample type", xlim=c(-1.5,1.5), ylim=c(-1,1.1))
...
points(meta.full, select=which(fact_site.full[,1]=="GW" & fact_time.full[,1]=="May"),col="blue",pch=22)
points(meta.full, select=which(fact_site.full[,1]=="GW" & fact_time.full[,1]=="Oct"),col="blue",pch=15)
...
ordispider(meta.full, group=fact_type.full[,1],col=c("blue","orange","green"),label=F)
ordiellipse(meta.full, group=fact_type.full[,1],kind="ehull",lty=c(1,3,4),label=T,lwd=1)
text(0, -1, label=paste0("Stress: ",round(meta.full$stress, 6)))
#dev.off()
```


# 6. Niche/Diet breadth
...
breadth.dv = as.data.frame(abund.guts[,!c(colnames(abund.guts) %in% c("time","sample_type","site"))] %>% group_by(predator_id) %>% summarise_all(funs(sum)))
