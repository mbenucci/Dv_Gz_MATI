...
# gut contents PERMANOVA
adonis.guts = abund.guts[,!c(colnames(abund.guts) %in% c("time","site","type","predator_id"))]
...
fact_all.guts = matrix(c(abund.guts$site, abund.guts$time, abund.guts$predator_id), ncol=3)
...
adonis(adonis.guts~fact_predator.guts*fact_time.guts*fact_site.guts, permutations = 1000, method="euclidean")
```

```{r nmds based on distances and on abundance data}
# NMDS based on the abundance data (will give the species scores as well)
meta.ab.guts = metaMDS(adonis.guts, trymax=200, distance="euclidean")
```


```{r plotting guts nmds}
nmds.guts = as.data.frame(scores(meta.ab.guts))
nmds.guts$id = rownames(nmds.guts)
nmds.guts$site = abund.guts[,"site"]
nmds.guts$predator = abund.guts[,"predator_id"]
nmds.guts$time = abund.guts[,"time"]

nmds.sp.guts = as.data.frame(scores(meta.ab.guts,"species"))
nmds.sp.guts$species = rownames(nmds.sp.guts)

# full symbols for May, hollow symbols for October
png("images/guts_predator_NMDS_euclidean.png", width = 800, height = 800, units = "px")
plot(meta.ab.guts, type="n", main="gut contents NMDS")#, xlim=c(-2,1.5), ylim=c(-1.2,1.2))
points(meta.ab.guts, select=which(fact_site.guts[,1]=="WB" & fact_time.guts[,1]=="May"),col="black",pch=20)
points(meta.ab.guts, select=which(fact_site.guts[,1]=="WB" & fact_time.guts[,1]=="Oct"),col="black",pch=1)
points(meta.ab.guts, select=which(fact_site.guts[,1]=="GW" & fact_time.guts[,1]=="May"),col="blue",pch=22)
points(meta.ab.guts, select=which(fact_site.guts[,1]=="GW" & fact_time.guts[,1]=="Oct"),col="blue",pch=15)
points(meta.ab.guts, select=which(fact_site.guts[,1]=="RB" & fact_time.guts[,1]=="May"),col="red",pch=17)
points(meta.ab.guts, select=which(fact_site.guts[,1]=="RB" & fact_time.guts[,1]=="Oct"),col="red",pch=24)
ordispider(meta.ab.guts,group=fact_predator.guts[,1],col=c("blue","orange"),label=T)
ordiellipse(meta.ab.guts,group=fact_predator.guts[,1],kind="ehull",lty=c(1,5),label=F,lwd=1)
dev.off()
```
...
dist.edna = vegdist(adonis.edna, method="bray")

adonis(adonis.edna~fact_site.edna*fact_time.edna*fact_id.edna, permutations = 10000, method="bray")
adonis(dist.edna~fact_site.edna*fact_time.edna*fact_id.edna, permutations = 10000, method="bray")
```

```{r nmds based on distances and on abundance data}
# NMDS based on the distance matrix
meta.edna = metaMDS(dist.edna, distance="bray")

# NMDS based on the abundance data (will give the species scores as well)
meta.ab.edna = metaMDS(adonis.edna, distance="bray")
```


```{r plotting eDNA nmds}
nmds.edna = as.data.frame(scores(meta.ab.edna))
nmds.edna$id = rownames(nmds.edna)
nmds.edna$site = abund.edna[,"site"]
nmds.edna$time = abund.edna[,"time"]
nmds.edna$sample = abund.edna[,"sample_id"]

nmds.sp.edna = as.data.frame(scores(meta.ab.edna,"species"))
nmds.sp.edna$species = rownames(nmds.sp.edna)

# full symbols for May, hollow symbols for October
png("images/eDNA_NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
plot(meta.ab.edna, type="n", main="eDNA NMDS", xlim=c(-1,1.5), ylim=c(-1,1))
points(meta.ab.edna, select=which(fact_site.edna[,1]=="WB" & fact_time.edna[,1]=="May"),col="black",pch=20)
points(meta.ab.edna, select=which(fact_site.edna[,1]=="WB" & fact_time.edna[,1]=="Oct"),col="black",pch=1)
points(meta.ab.edna, select=which(fact_site.edna[,1]=="GW" & fact_time.edna[,1]=="May"),col="blue",pch=22)
points(meta.ab.edna, select=which(fact_site.edna[,1]=="GW" & fact_time.edna[,1]=="Oct"),col="blue",pch=15)
points(meta.ab.edna, select=which(fact_site.edna[,1]=="RB" & fact_time.edna[,1]=="May"),col="red",pch=17)
points(meta.ab.edna, select=which(fact_site.edna[,1]=="RB" & fact_time.edna[,1]=="Oct"),col="red",pch=24)
ordispider(meta.ab.edna, group=fact_site.edna[,1],col=c("blue","orange","green"),label=T)
ordiellipse(meta.ab.edna, group=fact_site.edna[,1],kind="ehull",lty=c(1,3,5),label=F,lwd=1)
dev.off()
```
...
```{r nmds based on distances and on abundance data}
# NMDS based on the distance matrix
# meta.guts.edna = metaMDS(dist.full, trymax=200, distance="bray")
...
# full symbols for May, hollow symbols for October
png("images/kick-samples_NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
...
```{r}
# full symbols for May, hollow symbols for October
png("images/kick-eDNA_NMDS_bray-curtis.png", width = 800, height = 800, units = "px")
plot(meta.kick.edna, type="n", main="kick-samples NMDS")#, xlim=c(-2.5,5), ylim=c(-1,1))
...
niche.model = as.data.frame(niche.dv[,!c(colnames(niche.dv) %in% c("sample_type","sample_id"))] %>% group_by(site, time, predator_id) %>% summarise_all(funs(sum)))
...
niche.nmds = as.data.frame(niche.dv)

niche.nmds = subset(niche.nmds, subset=rowSums(niche.nmds[,!c(colnames(niche.dv) %in% c("time","site","predator_id","sample_type"))]) > 0)

#matrix.niche = as.matrix(niche.nmds[,!c(colnames(niche.nmds) %in% c("time","site","predator_id","sample_type"))])
...
dist.niche = vegdist(niche.dv, method="euclidean")
