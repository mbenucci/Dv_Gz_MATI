...
# test.edna = as.data.frame(t(abund.edna))
# abund.edna$genus = gsub("([_]{1}[A-Za-z0-9-]{+}.{+}[.]{0,1}){0,+}$","", rownames(test.edna))

melt.abund.edna = melt(abund.edna)
melt.abund.edna$genus = gsub("([_]{1}[A-Za-z0-9-]{+}.{+}[.]{0,1}){0,+}$","", melt.abund.edna$variable)

## FROM HERE!!!!
# NOTE: to find the right way to plot this data! (barplot?) Also to implement, alpha-div and raref. curves (need to transpose and sort samle names!) plus overlap between guts and eDNA. Next section kick-samples.

edna_inext = iNEXT(test.abund.edna, q=c(0,1), datatype = "abundance", se=T, endpoint=100)
...
ggplot(melt.abund.edna, aes(site, value)) +
  geom_bar(stat="identity", aes(colour=genus))
```
