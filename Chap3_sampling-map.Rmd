...
png("maps/Site_locations.png", width=3000, height=4000, units="px", res=300)
ggplot(uk_map) +
  ...
  coord_fixed(2) +
  theme_void() +
  north(uk_map, symbol=12, location="topleft")
dev.off()
```
...
png("maps/map_multiplot.png", width=6000, height=6000, units="px", res=300)
uk = ggplot(uk_map) +
  geom_polygon(aes(long,lat,group = group), fill="white", color="black") +
...
gw = ggplot(gw_map) +
  ...
  theme_void() +
  blank() +
  ggsn::scalebar(location="bottomleft", x.min=-0.342, x.max=-0.33, y.min=52.286, y.max=52.289, dist=600, dist_unit="m", st.dist=0.2, height=0.15, transform=T) +
